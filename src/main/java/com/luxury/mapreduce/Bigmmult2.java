...
public class Bigmmult2 {
	...
	public static class MapClass extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException,
			...
			StringTokenizer token = new StringTokenizer(line, CONTROL_I);
			if (pathName.contains(A)) {
				int j = 1; // 向量序号
				int rowindex = iA;
				while (token.hasMoreTokens()) {
					int colindex = j;
					String tmpValue = token.nextToken();
					if ("".equals(tmpValue)) {
						j++;
						continue;
					}
					int elevalue = Integer.parseInt(tmpValue);
			...
			if (pathName.contains(B)) {
				...
				while (token.hasMoreTokens()) {
					int colindex = j;
					int elevalue = Integer.parseInt(token.nextToken());
	...
	public static void main(String[] args) throws Exception {
		JobConf conf = new JobConf(Bigmmult2.class);
		conf.setJobName("Bigmmult2_MapReduce");
